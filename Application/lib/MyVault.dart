...
class _MyVaultScreenState extends State<MyVaultScreen>
  ...
  String _selectedCategory = 'All'; // Will be updated when context is available
  ...
  List<String> _getCategories(BuildContext context) {
    return [
      ...
      AppLocalizations.of(context)!.cropHistory,
      AppLocalizations.of(context)!.invoices,
      AppLocalizations.of(context)!.landDocuments,
      AppLocalizations.of(context)!.agriLoanRecords,
    ];
  }
  ...
  void _navigateToProfile({bool fromTutorial = false}) async {
  ...
  Future<void> _initializeSession() async {
    try {
      ...
      if (!_sessionManager.isAuthenticated()) {
        debugPrint(
          'No valid session found, attempting to restore from storage',
        );
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          backgroundColor:
              message.contains('Error') ? Colors.red : const Color(0xFF0A9D88),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    // Ensure selectedCategory is set to localized "All" if it's still the default
    ...
    if (_showTutorial) {
      return TutorialWrapper(
        showTutorial: true,
        initialStep: 2, // Start at step 2 (Complete Your Profile)
        onTutorialComplete: () {
          setState(() {
            _showTutorial = false;
          });
        },
        onNavigateToProfile: () => _navigateToProfile(fromTutorial: true),
        child: scaffold,
      );
    }
  ...
  Widget _buildDocumentsList() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              AppLocalizations.of(context)!.pleaseLoginToViewDocuments,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              AppLocalizations.of(context)!.userIdNotFound,
              style: const TextStyle(fontSize: 16, color: Colors.grey),
            ),
          ],
        ),
    ...
    // Add TabBar to switch between Documents and Activity Logs
    return DefaultTabController(
      length: 2,
      child: Column(
        children: [
          TabBar(
            labelColor: const Color(0xFF0A9D88),
            unselectedLabelColor: Colors.grey,
            indicatorColor: const Color(0xFF0A9D88),
            tabs: const [Tab(text: 'Documents'), Tab(text: 'Activity Logs')],
          ),
          Expanded(
            child: TabBarView(
              children: [
                _buildDocumentsTab(userId),
                _buildActivityLogsTab(userId),
              ],
            ),
          ),
        ],
  ...
  Widget _buildDocumentsTab(String userId) {
    return StreamBuilder<List<DocumentModel>>(
      ...
      builder: (context, snapshot) {
        ...
        // Filter documents
        final allCategoryText = AppLocalizations.of(context)!.all;
  ...
  Widget _buildActivityLogsTab(String userId) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: FarmActivityService.instance.getUserActivities(
        limit: 50,
        userId: userId,
      ),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return _buildActivityLogsShimmer();
        }
        ...
        return RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              return _buildActivityLogCard(activities[index]);
            },
          ),
        );
      },
  ...
  Widget _buildActivityLogCard(Map<String, dynamic> activityData) {
    final activity = activityData['activity']?.toString() ?? 'Unknown Activity';
    final notes = activityData['notes']?.toString() ?? '';
    final isCompleted = activityData['isCompleted'] ?? false;
    final timestamp = activityData['timestamp'];
    final activityType = activityData['activityType']?.toString() ?? 'general';
    final extractedData =
        (activityData['extractedData'] as Map<String, dynamic>?) ?? {};
    ...
    // Extract key fields from extractedData for display
    String? cropHarvested = extractedData['cropHarvested']?.toString();
    String? quantity = extractedData['quantity']?.toString();
    String? location = extractedData['location']?.toString();
    String? duration = extractedData['duration']?.toString();
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            ...
            colors: [
              _getActivityTypeColor(activityType).withOpacity(0.05),
              Colors.white,
            ],
            stops: const [0.0, 0.3],
          ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        DateFormat('MMM dd').format(activityDate),
                        style: TextStyle(
                          fontSize: 12,
                      ...
                      Text(
                        DateFormat('HH:mm').format(activityDate),
              ...
              const SizedBox(height: 12),
              // Display extracted key data if available
              if (cropHarvested != null ||
                  quantity != null ||
                  location != null ||
                  duration != null)
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (cropHarvested != null)
                        _buildDetailRow(Icons.grain, 'Crop', cropHarvested),
                      if (quantity != null)
  ...
  Widget _buildActivityLogsShimmer() {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed:
                  () => Navigator.pushNamed(context, '/daily-activities'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _selectedCategory == AppLocalizations.of(context)!.all
                  ? AppLocalizations.of(context)!.uploadFirstDocument
                  : AppLocalizations.of(
                    context,
                  )!.noDocumentsInCategory(_selectedCategory),
              style: TextStyle(fontSize: 16, color: Colors.grey[600]),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              ...
              label: Text(AppLocalizations.of(context)!.uploadDocument),
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF0A9D88),
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(
