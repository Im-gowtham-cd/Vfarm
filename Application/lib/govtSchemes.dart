...
class EnhancedGovtSchemesScreen extends StatefulWidget {
  ...
  @override
  State<EnhancedGovtSchemesScreen> createState() =>
      _EnhancedGovtSchemesScreenState();
}
...
class _EnhancedGovtSchemesScreenState extends State<EnhancedGovtSchemesScreen>
  ...
  Future<void> _initializeData() async {
    try {
      ...
      // Remove duplicates based on scheme ID
      final seenIds = <String>{};
      _allSchemes =
          _allSchemes.where((scheme) {
            if (seenIds.contains(scheme.id)) {
              return false;
            }
            seenIds.add(scheme.id);
            return true;
          }).toList();
      print('Schemes after removing duplicates: ${_allSchemes.length}');

      // Show ALL schemes (both eligible and not eligible)
      _eligibleSchemes = _allSchemes;
      print('Showing all schemes (eligible and not eligible)');
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: l10n.searchSchemesByName,
                                ...
                                _categories.map((category) {
                                  ...
                                  switch (category) {
                                    case 'All':
                                      ...
                                      break;
                                    case 'Subsidy':
                                      displayText = l10n.subsidy;
                                      break;
                                    case 'Loan':
                                      displayText = l10n.loan;
                                      break;
                                    case 'Insurance':
                                      displayText = l10n.insurance;
                                      break;
                                    case 'Training':
                                      displayText = l10n.training;
                                      break;
                                    case 'Technology':
                                      displayText = l10n.technology;
                                      break;
                                    default:
                                ...
                                _statusFilters.map((status) {
                                  ...
                                  switch (status) {
                                    case 'All':
                                      ...
                                      break;
                                    case 'Eligible':
                                      displayText = l10n.eligible;
                                      break;
                                    case 'Not Eligible':
                                      displayText = l10n.notEligible;
                                      break;
                                    default:
                                      displayText = status;
                                  }
  ...
  Widget _buildSchemesTab(AppLocalizations l10n) {
    return Column(
      children: [
                            ...
                            : RefreshIndicator(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return _buildEnhancedSchemeCard(
                                    scheme,
                                    isEligible,
                                  );
                                },
  ...
  bool _isUserEligible(GovtSchemeModel scheme) {
    ...
    bool cropEligible = true;
    if (scheme.eligibleCropTypes.isNotEmpty) {
      cropEligible =
          scheme.eligibleCropTypes.contains('all') ||
          _userProfile!.cropTypes.any(
            (userCrop) => scheme.eligibleCropTypes.any(
              (eligibleCrop) =>
    ...
    // Check farm size (acres) eligibility
    bool farmSizeEligible = true;
    if (_userProfile!.farmSize != null &&
        scheme.eligibilityCriteria.isNotEmpty) {
      final minLandHolding = scheme.eligibilityCriteria['minLandHolding'];
      final maxLandHolding = scheme.eligibilityCriteria['maxLandHolding'];

      if (minLandHolding != null) {
        final minLand = minLandHolding is num ? minLandHolding.toDouble() : 0.0;
        if (_userProfile!.farmSize! < minLand) {
          farmSizeEligible = false;
        }
      }

      if (maxLandHolding != null && farmSizeEligible) {
        final maxLand =
            maxLandHolding is num ? maxLandHolding.toDouble() : double.infinity;
        if (_userProfile!.farmSize! > maxLand) {
          farmSizeEligible = false;
        }
      }
    }

    // User is eligible only if both crop and farm size criteria match
    return cropEligible && farmSizeEligible;
  }
  ...
  Widget _buildSideNavigation() {
    return Drawer(
      child: Column(
        children: [Expanded(child: ModernSideMenu(currentRoute: ''))],
      ),
    );
  }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () {
                          if (!isEligible) {
                            // Show warning for non-eligible schemes
                            showDialog(
                              context: context,
                              builder:
                                  (context) => AlertDialog(
                                    ...
                                    title: const Row(
                                      children: [
                                        ...
                                        SizedBox(width: 12),
                                        Expanded(child: Text('Not Eligible')),
                                      ],
                                    ),
                                    content: const Text(
                                      'You may not meet all the eligibility criteria for this scheme. '
                                      'You can still apply, but your application might be rejected. '
...
class SchemeDescriptionWithVoice extends StatefulWidget {
  ...
  @override
  State<SchemeDescriptionWithVoice> createState() =>
      _SchemeDescriptionWithVoiceState();
}

class _SchemeDescriptionWithVoiceState
    extends State<SchemeDescriptionWithVoice> {
  ...
  Widget build(BuildContext context) {
    ...
    final isEligible = widget.isEligible;
    final onApplicationSubmitted = widget.onApplicationSubmitted;
            ...
            scheme.imagePath.isNotEmpty
                ? ClipRRect(
                  ...
                  child: CachedNetworkImage(
                    ...
                    errorWidget: (context, error, stackTrace) {
                      print('Error loading scheme image: $error');
                      return const Center(
                        child: Icon(
                          Icons.agriculture_rounded,
                          size: 80,
